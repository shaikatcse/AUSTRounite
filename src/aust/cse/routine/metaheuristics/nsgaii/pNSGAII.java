...
public class pNSGAII extends Algorithm {
	...
	public pNSGAII(Problem problem, String folderForGenerationData) {
		super(problem);
		file = new File(folderForGenerationData);
		file.mkdirs();
	} // NSGAII
	
	public pNSGAII(Problem problem, String folderForGenerationData, IParallelEvaluator evaluator) {
		super(problem);
			...
			if (evaluations % 21000 == 0) {
				for (int i = 0; i < population.size(); i++) {
					...
					System.out.println("Obj1: " + population.get(i).getObjective(0) + "Obj2: "
							+ population.get(i).getObjective(1));
				}
				System.out.println();
			}
